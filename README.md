...
### เชิญเพื่อน (invite / referrer) — ยังไม่พร้อมขาย

> **สถานะ: พับไว้** ทดสอบกับเซิร์ฟเวอร์จริงแล้ว `SetReferrer` สำเร็จ 100% และเซิร์ฟเวอร์บันทึกผู้เชิญจริง
> **แต่ยอด "เชิญเพื่อน" ในเกมไม่ขึ้น เพราะบัญชีที่ถูกเชิญต้องถึงเลเวล 5 ก่อนถึงจะนับ**
> guest ที่สร้างใหม่อยู่เลเวล 1 และไม่มีทางได้ EXP โดยไม่เล่นจริง — ตรวจแล้วว่า
> `GetMemberMissionInfos` ไม่มีภารกิจที่จบแล้ว และ `ClaimQuestRewardAll` คืน level 1
> ไม่จ่าย EXP ให้เลย (guest ได้แค่ของเริ่มต้น: เหรียญ 210,000 หัวใจ 5 กุญแจ 33)
>
> EXP มาจาก `EpisodeAPI.EpisodeAfterPlay` ทางเดียว ซึ่งมี `integrity_check_hashes`
> + `replay_data` ให้เซิร์ฟเวอร์ตรวจว่าผลการเล่นเป็นของจริง — อย่าเปิดขายเมนูนี้
> จนกว่าจะมีคำตอบว่าจะข้ามข้อนี้ยังไงโดยไม่ปลอมผลการเล่น ลูกค้าจะเห็นยอดเป็น 0
...
guest ต้องมีโปรไฟล์ก่อนถึงจะตั้ง referrer ได้ (ไม่งั้นเซิร์ฟเวอร์ตอบ `ILLEGAL PARAMETER`) ตัวเครื่องมือจึงยิง `SetMemberProfile` ด้วยชื่อเล่น/ภูมิภาคสุ่มให้ทุกตัวก่อนเสมอ
