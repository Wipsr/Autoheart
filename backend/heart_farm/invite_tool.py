...
============================================================================
 ...
 !! ข้อจำกัดที่เจอจากการทดสอบกับเซิร์ฟเวอร์จริง (สำคัญ อ่านก่อนเอาไปใช้):
   การตั้ง referrer สำเร็จไม่ได้แปลว่ายอด "เชิญเพื่อน" ของผู้เชิญจะขึ้น — บัญชีที่
   ถูกเชิญต้องถึง "เลเวล 5" ก่อนถึงจะถูกนับ ส่วน guest ที่สร้างใหม่อยู่เลเวล 1 และ
   ไม่มีทางได้ EXP โดยไม่เล่นจริง (ClaimQuestRewardAll ไม่จ่าย EXP ให้บัญชีใหม่เลย
   เพราะไม่มีภารกิจไหนจบ) EXP มาจาก EpisodeAfterPlay ทางเดียวซึ่งเซิร์ฟเวอร์ตรวจ
   ด้วย integrity_check_hashes + replay_data — เครื่องมือนี้จึงยัง "ใช้ได้ในระดับ
   โปรโตคอล แต่ยังไม่ให้ผลที่ลูกค้าเห็น" อย่าเปิดขายจนกว่าข้อนี้จะมีทางออก
