...
class MainAccount:
    def __init__(self, email, password):
        ...
        # wallet ของบัญชี เช่นยอดผงเวทมนตร์/เหรียญ อยู่ในก้อนนี้ที่เดียว —
        # shop_recon.py (เครื่องมือสำรวจ) ใช้อ่านค่านี้ ส่วนการฟาร์มหัวใจไม่ได้ใช้
        # และไม่ได้รับผลกระทบ
    ...
    while total < target_hearts:
        ...
        if API_MODE:
            _api_progress(
                total,
                target_hearts,
                "SESSION %d: +%d (total %d/%d)" % (session_num, collected, total, target_hearts),
...
def _emit(obj):
    """Emit a single JSON line for the FastAPI worker to parse (api-mode only)."""
    if not API_MODE:
        return
...
def _api_log(message, level="info", **data):
    payload = {"type": "log", "level": level, "message": message}
    if data:
        payload["data"] = data
    _emit(payload)
    # Also mirror human-readable for local debugging
    print(message)


def _api_progress(hearts_collected, target_hearts, message="", **extra):
    payload = {
        "type": "progress",
        "hearts_collected": int(hearts_collected),
        "target_hearts": int(target_hearts),
        "message": message or ("%d/%d" % (hearts_collected, target_hearts)),
    }
...
def api_main(email, password, target_hearts, proxy=""):
    """Non-interactive entry used by Autoheart backend worker."""
    global MAIN_EMAIL, MAIN_PASSWORD, TARGET_HEARTS, PROXY_URL, _PROXIES, API_MODE, _API_TARGET
    API_MODE = True
    MAIN_EMAIL = email
    MAIN_PASSWORD = password
    TARGET_HEARTS = int(target_hearts)
    _API_TARGET = TARGET_HEARTS
    PROXY_URL = proxy or ""
    _PROXIES = {"http": PROXY_URL, "https": PROXY_URL} if PROXY_URL else None

    _api_log("Starting heart farm (api-mode)", email=MAIN_EMAIL, target=TARGET_HEARTS, proxy=bool(PROXY_URL))
    _api_progress(0, TARGET_HEARTS, "กำลังล็อกอิน DevPlay...")

    main_acct = MainAccount(MAIN_EMAIL, MAIN_PASSWORD)
    try:
        total = run_until_target(main_acct, TARGET_HEARTS)
        ok = total >= TARGET_HEARTS
        _api_progress(total, TARGET_HEARTS, "เสร็จสิ้น" if ok else "จบก่อนครบเป้า")
        _emit({"type": "done", "ok": ok, "hearts_collected": int(total), "target_hearts": TARGET_HEARTS})
        if not ok:
            raise SystemExit(2)
    finally:
        main_acct.close()
...
def _parse_args(argv=None):
    import argparse
    p = argparse.ArgumentParser(description="Cookie Run Heart Farm")
    p.add_argument("--api-mode", action="store_true", help="JSON progress lines for Autoheart worker")
    p.add_argument("--email", default="")
    p.add_argument("--password", default="")
    p.add_argument("--target-hearts", type=int, default=0)
    ...
    try:
        if args.api_mode:
            if not args.email or not args.password:
                _emit({"type": "error", "message": "email/password required in api-mode"})
                raise SystemExit(2)
            target = args.target_hearts or TARGET_HEARTS
            api_main(args.email, args.password, target, args.proxy or "")
        else:
            main()
    except SystemExit as e:
        if API_MODE and int(getattr(e, "code", 1) or 1) not in (0, None):
            _emit({"type": "error", "message": str(e)})
        else:
            print(e)
    except KeyboardInterrupt:
        msg = "หยุดโดยผู้ใช้ (Ctrl+C)"
        if API_MODE:
        ...
        if API_MODE:
            _emit({"type": "error", "message": str(e), "data": {"trace": traceback.format_exc()[-2000:]}})
        else:
            traceback.print_exc()
    if not API_MODE:
