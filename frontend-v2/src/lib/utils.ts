...
/** ยอดพอยท์ในกระเป๋า ราคาแพ็ก และยอดที่หัก/คืน */
export function formatPoints(n: number) {
  return new Intl.NumberFormat("th-TH").format(n);
}

/** หัวใจในเกมที่ worker เก็บมาได้จริง — คนละเรื่องกับพอยท์ ถึงอัตราจะ 1:1 */
export function formatHearts(n: number) {
  return new Intl.NumberFormat("th-TH").format(n);
}

/** จำนวนนับทั่วไป (เพื่อน คำขอ รายการ) ที่ไม่ใช่ทั้งพอยท์และหัวใจ */
