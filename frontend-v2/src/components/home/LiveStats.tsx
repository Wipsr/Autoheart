...
export function LiveStats() {
  ...
  return (
    <div className="flex w-full flex-wrap items-center gap-x-8 gap-y-4 rounded-card border border-line bg-surface px-5 py-4 shadow-card">
      <div className="flex items-center gap-2 font-mono text-xs">
        <span
          className={cn(
            "inline-block h-2 w-2 rounded-full",
            stats.paused ? "bg-point" : "pulse-dot bg-live"
          )}
        />
        <span className={stats.paused ? "text-point-ink" : "text-live-ink"}>
          {stats.paused ? "คิวหยุดชั่วคราว" : "ระบบทำงานปกติ"}
        </span>
      </div>
      <Stat label="กำลังรัน" value={String(stats.running_count)} tone="text-live-ink" />
      <Stat label="รอในคิว" value={String(stats.queued_count)} tone="text-wait-ink" />
      <Stat
        label="รอโดยประมาณ"
