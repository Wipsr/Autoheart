...
export default function FriendRequestsPage() {
  ...
  const busyLabel = (verb: string) =>
    progress && progress.total > BATCH_SIZE
      ? `${verb} ${formatNumber(progress.done)} / ${formatNumber(progress.total)}...`
      : `${verb}...`;
    ...
    <div className="mx-auto max-w-3xl space-y-5">
      ...
      {phase === "list" && (
        <>
          <Card className="flex flex-wrap items-center justify-between gap-3 p-4">
            <div className="text-sm">
              ...
              <p className="text-dim">
                เพื่อนตอนนี้ {formatNumber(friendCount)}
                {friendCap ? ` / ${formatNumber(friendCap)}` : ""}
                {slots != null && ` · รับเพิ่มได้อีก ${formatNumber(slots)}`}
                {guestCount > 0 && ` · น่าจะเป็นบัญชี guest ${formatNumber(guestCount)}`}
              </p>
            </div>
          ...
          {result && (
            <Card className="border-live/40 bg-live/[0.04] p-4 text-sm">
              <p className="font-medium">
                {result.kind === "accept"
                  ? `รับสำเร็จ ${formatNumber(result.data.accepted)} / ${formatNumber(result.data.requested)} คำขอ`
                  : `ปฏิเสธแล้ว ${formatNumber(result.data.rejected)} / ${formatNumber(result.data.requested)} คำขอ`}
              </p>
              {result.kind === "accept" && result.data.skipped_cap > 0 && (
                <p className="mt-1 text-muted">
                  ข้าม {formatNumber(result.data.skipped_cap)} คำขอเพราะเพื่อนเต็มเพดานแล้ว —
                  ลบเพื่อนออกก่อนแล้วค่อยกดรับใหม่
                </p>
              )}
              {result.data.skipped_not_pending > 0 && (
                <p className="mt-1 text-muted">
                  ข้าม {formatNumber(result.data.skipped_not_pending)} รายชื่อที่ไม่มีคำขอค้างอยู่แล้ว
                </p>
              )}
          ...
          <Card>
            <CardHead
              title={`คำขอเป็นเพื่อน (${formatNumber(requests.length)})`}
              action={
                requests.length > 0 && (
            ...
            {requests.length === 0 ? (
              ...
              <ul className="divide-y divide-lineSoft">
                {requests.map((r) => (
                  <li key={r.player_id}>
                    <label className="flex cursor-pointer items-center gap-3 px-4 py-2.5 transition hover:bg-subtle">
                      <input
                        type="checkbox"
              ...
              {confirmingReject ? (
                <>
                  <p className="flex items-start gap-2 text-sm text-fail">
                    ...
                    ปฏิเสธ {formatNumber(selected.size)} คำขอ? คำขอจะหายไปจากลิสต์ —
                ...
                <>
                  {overCap && (
                    <p className="flex items-start gap-2 text-sm text-fail">
                      <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
                      เลือกไว้ {formatNumber(selected.size)} คน แต่รับเพิ่มได้อีกแค่{" "}
                      {formatNumber(slots ?? 0)} — ส่วนที่เกินจะถูกข้าม
                    </p>
                  )}
                  <div className="flex flex-wrap gap-2">
                    <Button onClick={runAccept} disabled={busy || selected.size === 0}>
                      {accepting ? (
                        ...
                        <>
                          ...
                          {allSelected
                            ? `รับทั้งหมด (${formatNumber(selected.size)})`
                            : `รับที่เลือก (${formatNumber(selected.size)})`}
                        </>
                      )}
                    </Button>
                    <Button
                    ...
                    >
                      ...
                      {allSelected
                        ? `ปฏิเสธทั้งหมด (${formatNumber(selected.size)})`
                        : `ปฏิเสธที่เลือก (${formatNumber(selected.size)})`}
                    </Button>
                  </div>
                </>
              )}
