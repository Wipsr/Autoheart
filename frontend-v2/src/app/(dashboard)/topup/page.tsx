...
export default function TopupPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <h1 className="font-display text-2xl font-semibold">เติม / แลกเป็นหัวใจ</h1>
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        <PointChip points={pointBalance} suffix="พอยท์คงเหลือ" />
        <PointChip points={heartBalance} suffix="หัวใจคงเหลือ" />
      </div>
      ...
      <Card className="overflow-hidden">
        ...
        <form onSubmit={submitTopup} className="space-y-3 p-5">
          ...
          {topupError && <p className="text-sm text-fail-ink">{topupError}</p>}
          {topupResult && (
            <p className="text-sm text-live-ink">
              เติมสำเร็จ +{formatPoints(topupResult.points_credited)} พอยท์ — ยอดพอยท์ล่าสุด{" "}
        ...
        <form onSubmit={submitConvert} className="space-y-4 p-5">
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-lg border p-3 text-left transition ${
                    active ? "border-point-line bg-point-soft" : "border-line bg-subtle"
                  }`}
                >
                  <div>
                    <p className="text-sm font-medium">{pkg.name}</p>
                    <p className="mt-0.5 font-mono text-xs tabular-nums text-dim">
                      {formatPoints(pkg.price_baht)} พอยท์ → {formatPoints(pkg.points)} หัวใจ
                    </p>
                  </div>
                  {pkg.badge && (
                    <span className="rounded border border-lineStrong px-1.5 py-0.5 text-[10px] text-dim">
                      {pkg.badge}
                    </span>
                  )}
                </button>
          ...
          {selectedPkg && (
            <>
              ...
              <div className="flex items-center justify-between gap-3 rounded-card border border-point-line bg-point-soft p-4 text-sm">
                <div className="space-y-1">
                  <p className="text-point-dim">
                    ใช้พอยท์{" "}
                    <span className="font-mono tabular-nums text-ink2">
                      {formatPoints(pointsCost)}
                    </span>
                  </p>
                  <p className="text-point-dim">
                    ได้หัวใจ{" "}
                    <span className="font-mono tabular-nums text-point-ink">
                      {formatPoints(heartsGain)}
                    </span>
                  </p>
                </div>
                <ArrowRight className="h-4 w-4 shrink-0 text-point" />
              </div>

              {!canConvert && pointsCost > 0 && (
                <p className="text-xs text-fail-ink">
                  พอยท์ไม่พอ — มี {formatPoints(pointBalance)} ต้องการ {formatPoints(pointsCost)}
                </p>
              )}
              {convertError && <p className="text-sm text-fail-ink">{convertError}</p>}
              {convertResult && (
                <p className="text-sm text-live-ink">
                  แลกสำเร็จ −{formatPoints(convertResult.points_spent)} พอยท์ +
                  {formatPoints(convertResult.hearts_credited)} หัวใจ
                </p>
              )}
              ...
              <Button type="submit" className="w-full" disabled={!canConvert || convertLoading}>
                {convertLoading ? (
                  ...
                  `แลก ${formatPoints(pointsCost)} พอยท์ เป็น ${formatPoints(heartsGain)} หัวใจ`
                )}
              </Button>
            </>
          )}
