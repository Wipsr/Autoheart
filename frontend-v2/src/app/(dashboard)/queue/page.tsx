...
import { QueueStrip } from "@/components/queue/QueueStrip";
...
export default function QueuePage() {
  const { token } = useAuth();
  const { jobs, queuedCount, activeJob, loading, lastUpdated, refresh } = useQueue();
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-semibold">คิวงาน</h1>
      ...
      <QueueStrip
        ...
        activeJob={activeJob}
        yourPositions={jobs
          .filter((j) => j.status === "queued" && j.queue_position != null)
          .map((j) => j.queue_position as number)}
        ...
        <Card className="overflow-hidden">
          ...
          {!loading && jobs.length === 0 && (
            <EmptyState
              ...
              description="เติมพอยท์แล้วสั่งรันได้เลย ระบบจะตรวจไอดีให้ก่อนโดยไม่หักพอยท์"
