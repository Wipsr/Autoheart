    ...
    <div className="mx-auto max-w-3xl space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          ล็อกอินไอดี DevPlay เพื่อดูรายชื่อเพื่อนทั้งหมด แล้วเลือกลบได้เลย — ฟรี ไม่ใช้พอยท์
          และไม่ต้องรอคิว
        </p>
      </div>
      ...
      {phase === "list" && (
        <>
          <Card className="flex flex-wrap items-center justify-between gap-3 p-4">
            <div className="text-sm">
              ...
              <p className="text-dim">
                เพื่อนทั้งหมด {formatNumber(friends.length)}
                {account?.friend_cap ? ` / ${formatNumber(account.friend_cap)}` : ""}
          ...
          {result && (
            ...
            >
              <p className="font-medium">
                ลบสำเร็จ {formatNumber(result.removed)} / {formatNumber(result.requested)} รายชื่อ
              </p>
          ...
          <Card>
            <CardHead
              title={`รายชื่อเพื่อน (${formatNumber(friends.length)})`}
              action={
                friends.length > 0 && (
              ...
              {confirming ? (
                <>
                  <p className="flex items-start gap-2 text-sm text-fail">
                    ...
                    ยืนยันลบเพื่อน {formatNumber(selected.size)} คน? การลบกู้คืนไม่ได้
                ...
                >
                  ...
                  {allSelected
                    ? `ลบเพื่อนทั้งหมด (${formatNumber(selected.size)})`
                    : `ลบเพื่อนที่เลือก (${formatNumber(selected.size)})`}
                </Button>
              )}
            </Card>
          )}
