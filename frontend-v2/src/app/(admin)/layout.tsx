              ...
              <nav className="space-y-0.5">
                {group.links.map(([href, label]) => (
                  <Link
                    ...
                    aria-current={isActive(href) ? "page" : undefined}
                    className={cn(
                      "block rounded-lg px-3 py-1.5 text-sm text-muted transition hover:bg-surface2 hover:text-ink",
                      isActive(href) &&
                        "bg-surface2 font-medium text-ink shadow-[inset_2px_0_0_var(--point)]"
                    )}
                  >
                    {label}
                  </Link>
