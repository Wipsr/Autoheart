...
export default function AdminUsersPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="ผู้ใช้" description="ค้นหาผู้ใช้เพื่อดูพอยท์ งาน และสั่งแบน" />
        ...
        {!loading &&
          users.map((u) => (
            <Link
              ...
              className="flex items-center justify-between gap-3 border-b border-lineSoft px-4 py-3 text-sm transition last:border-b-0 hover:bg-subtle"
            >
              ...
              <span className="shrink-0 text-right font-mono text-xs tabular-nums">
                <span className="text-heart">{formatPoints(u.hearts)} หัวใจ</span>
                <span className="ml-2 text-dim">{formatPoints(u.points)} P</span>
              </span>
            </Link>
          ))}
      </Card>
